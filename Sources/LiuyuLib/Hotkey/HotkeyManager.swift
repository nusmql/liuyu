...
public class HotkeyManager {
    ...
    private(set) public var shortcut: RecordedShortcut = .default
    ...
    public func configure(shortcut newShortcut: RecordedShortcut) {
        Logger.debug("Configuring shortcut: \(newShortcut.displayString)", category: .hotkey)
        ...
        if newShortcut.isValid {
            do {
                ...
                Logger.error("Failed to restart hotkey runtime: \(error)", category: .hotkey)
            }
        }
    }
    ...
    public func start() throws {
        ...
        guard !isStarted else {
            Logger.debug("HotkeyManager already started", category: .hotkey)
            return
        }
        ...
        guard shortcut.isValid else {
            Logger.warning("No valid shortcut configured; hotkey manager not started", category: .hotkey)
            return
        }
    ...
    private func stopRuntime() {
        ...
        if let eventHandlerRef = eventHandlerRef {
            Logger.debug("Removing Carbon event handler", category: .hotkey)
            RemoveEventHandler(eventHandlerRef)
            self.eventHandlerRef = nil
            self.eventHandlerUPP = nil
        }
    ...
    private func handleFlagsChangedEvent(_ event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        let allMatch = ShortcutMatcher.matches(
            cgFlags: event.flags,
            keyCode: nil,
            shortcut: shortcut
        )

        Logger.debug("Flags changed: allMatch=\(allMatch), isKeyDown=\(isKeyDown), isRecording=\(isRecording), flags=\(event.flags.rawValue)", category: .hotkey)
    ...
    private func handleKeyEvent(_ event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if event.type == .keyDown {
            if ShortcutMatcher.matches(cgFlags: event.flags, keyCode: keyCode, shortcut: shortcut) {
            ...
            if keyCode == shortcut.keyCode && isKeyDown {
                isKeyDown = false
                events.send(.keyUp)
                return nil
            }
